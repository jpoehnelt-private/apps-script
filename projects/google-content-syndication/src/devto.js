...
function devTo({ id, snippet: { description, title, tags, thumbnails } }) {
    ...
    const article = DevTo.post(
      'https://dev.to/api/articles',
      {
        article: {
          title,
          description: description.split('\n')[0].substring(0, 250) + " ...",
          body_markdown: `{% youtube ${id} %}\n\n${description.replace(/#([a-zA-Z0-9]*)/ig, '[#$1](https://www.youtube.com/hashtag/$1)')}\n\n{% cta https://youtube.com/@googleworkspacedevs %}Follow youtube.com/@googleworkspacedevs{% endcta %}`,
          published: true,
          tags: [...tags, "googleworkspace", "google"].map(tag => tag.toLowerCase().replace(/[^a-z]*/ig, '')).slice(0, 4),
  ...
  class DevTo {
    ...
    static fetch(url, options, retries = DevTo.retries) {
      ...
      if (response.getResponseCode() === 429 && retries > 0) {
        ...
        return DevTo.fetch(url, options, retries = retries - 1)
      }
  ...
  DevTo.organizations = {
    articles: {
      list: (org = "googleworkspace") => DevTo.paginate(`https://dev.to/api/organizations/${org}/articles`)
    }
  };
