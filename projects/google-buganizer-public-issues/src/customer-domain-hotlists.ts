import { WorkspaceQuery, getBugUserEventDomains, getHotlistByDomain, domainHotlistName } from "@repository/buganizer-utils";
...
function addBugsToDomainHotlists(query: string) {
    const bugs = new WorkspaceQuery(query).search().sort(() => Math.random() - 0.5).slice(0, 100);
    ...
    for (const bug of bugs) {
        ...
        for (const domain of domains) {
            ...
            bug.getDependsOn().filter(dependency => BuganizerApp.getBug(dependency).isOpen()).forEach(dependency => {
                Logger.log({ ...memoizedAddBugToDomainHotlist(dependency, domain), blocks: bug.getId() });

                // Add dependencies of dependencies
                BuganizerApp.getBug(dependency).getDependsOn().filter(dependency => BuganizerApp.getBug(dependency).isOpen()).forEach(dependency2 => {
                    Logger.log({ ...memoizedAddBugToDomainHotlist(dependency2, domain), blocks: bug.getId() });
                });
            });

            const duplicate = bug.getDuplicateOf();
            if (duplicate && duplicate !== '0') {
                Logger.log({ ...memoizedAddBugToDomainHotlist(duplicate, domain), duplicates: bug.getId() });
            }
        }
        i++;
    }
...
const memoizedGetHotlistByDomain = memoize(getHotlistByDomain, { ttl: 600, cache: CacheService.getScriptCache(), cacheUndefined: false });



const memoizedAddBugToDomainHotlist = memoize(addBugToDomainHotlist, { ttl: 600, cache: CacheService.getScriptCache() });

function addBugToDomainHotlist(bugId: string, domain: string): { bugId: string, domain: string, hotlistId: string, message: string } {
    if (!bugId || !domain || Number.isNaN(Number(bugId)) || Number(bugId) <= 0) {
        return { bugId, domain, hotlistId: '', message: `Invalid bugId or domain: ${bugId}, ${domain}` };
    }
    ...
    if (!hotlistId) {
        ...
        if (!hotlistId) {
            return { bugId, domain, hotlistId: '', message: `Failed to create hotlist for domain ${domain}` };
        }
    }
