...
describe('BuganizerQuery', () => {
    ...
    describe('static search', () => {
        it('should call BuganizerApp.searchBugs with the query string', () => {
            const mockBugs = [{ getMeTooCount: () => 5 }, { getMeTooCount: () => 10 }] as unknown as GoogleAppsScript.Buganizer.Bug[];
            (global.BuganizerApp.searchBugs as unknown as ReturnType<typeof vi.fn>).mockReturnValue(mockBugs);
    ...
    describe('link', () => {
        it('should return the correct link', () => {
            ...
            expect(q.link()).toBe('https://issuetracker.google.com/issues?q=foo%3Abar');
        });
        it('should return the correct link static', () => {
            expect(BuganizerQuery.link('foo:bar')).toBe('https://issuetracker.google.com/issues?q=foo%3Abar');
        });
...
describe('WorkspaceQuery', () => {
    ...
    describe('minQuality', () => {
        it('should add minimum quality condition', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388853>=5');
        });
    });
    ...
    describe('maxQuality', () => {
        it('should add maximum quality condition', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388853<=8');
        });
    });
    ...
    describe('notClassroom', () => {
        it('should exclude classroom component', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar -componentid:191645');
        });
    });
    ...
    describe('isDeveloperIssue', () => {
        it('should add developer issue condition when true', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388852:1');
        });

        it('should add developer issue condition when false', () => {
            const q = query.isDeveloperIssue(false);
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388852:0');
        });
    });
    ...
    describe('isEnglish', () => {
        it('should add English language condition', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388884:1');
        });
    });
    ...
    describe('tag related methods', () => {
        it('isTaggedWith should add tag condition', () => {
            ...
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1172495:javascript');
        });

        it('isTaggedWithAny should add OR condition for tags', () => {
            const q = query.isTaggedWithAny(['javascript', 'typescript']);
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1172495:(javascript | typescript)');
        });
    ...
    describe('method chaining', () => {
        it('should support method chaining', () => {
            const q = query.minQuality(5).maxQuality(10).notClassroom().isDeveloperIssue(true);
            expect(q.toString()).toBe('componentid:191625+ foo:bar customfield1388853>=5 customfield1388853<=10 -componentid:191645 customfield1388852:1');
        });
