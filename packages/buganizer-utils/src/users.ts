...
export function getBugUserEventDomains(bugIds: string[] | string): string[] {
    return getBugUserEvents(bugIds).filter(Boolean).filter(filterWorkspaceDomain).map(u => u.split('@')[1])
}

export function filterWorkspaceDomain(email: string): boolean {
    return !/@.*(google.com|gmail.com|hotmail.com|outlook.com|live.com)$/i.test(email);
}
...
export function getHotlistByDomain(domain: string): string | undefined {
    ...
    const hotlists = BuganizerApp.searchHotlists(`owner:jpoehnelt '${name}'`).filter(h => h.getName() === name);
